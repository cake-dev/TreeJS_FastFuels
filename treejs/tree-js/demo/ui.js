...
export function setupUI(
  ...
  gui
    .add(
      {
        massCreateTrees: async () => {
          const directoryHandle = await window.showDirectoryPicker();
          const jsonFiles = await getJsonFilesFromDirectory(directoryHandle);
          const entries = Object.entries(jsonFiles);
          const totalBatches = Math.ceil(entries.length / 50);
  
          for (let batch = 0; batch < totalBatches; batch++) {
            const zip = new JSZip();
            const treeFolder = zip.folder("trees");
  
            const start = batch * 50;
            const end = Math.min((batch + 1) * 50, entries.length);
            const batchEntries = entries.slice(start, end);
  
            for (const [fileName, jsonContent] of batchEntries) {
              const treeOptions = JSON.parse(jsonContent);
              const newTree = new Tree();
              newTree.options = treeOptions;
              newTree.generate();
  
              // Extract SPCD and CR from the filename
              const match = fileName.match(/tree_spcd_(\d+)_cr_([\d.]+)\.json/);
              if (match) {
                const [, spcd, cr] = match;
                const treeId = `${spcd}_${cr}`;
                await exportTreeToZip(newTree, treeFolder, treeId);
              } else {
                console.warn(`Skipping file with unexpected name format: ${fileName}`);
              }
            }
  
            // Generate and download the zip for this batch
            const zipBlob = await zip.generateAsync({ type: 'blob' });
            const link = document.createElement('a');
            link.href = URL.createObjectURL(zipBlob);
            link.download = `trees_batch_${batch + 1}.zip`;
            link.click();
          ...
          alert("All trees have been processed and downloaded.");
        },
      },
      'massCreateTrees'
    )
    .name('Mass Create Trees');
...
async function getJsonFilesFromDirectory(directoryHandle) {
  const jsonFiles = {};
  for await (const entry of directoryHandle.values()) {
    if (entry.kind === 'file' && entry.name.endsWith('.json')) {
      const file = await entry.getFile();
      const content = await file.text();
      jsonFiles[entry.name] = content;
    }
  }
  return jsonFiles;
}

function exportTreeToZip(tree, zipFolder, treeId) {
  return new Promise((resolve, reject) => {
    exporter.parse(
      tree,
      (result) => {
        if (result instanceof ArrayBuffer) {
          zipFolder.file(`tree_${treeId}.glb`, result);
        } else {
          zipFolder.file(`tree_${treeId}.gltf`, JSON.stringify(result.gltf, null, 2));
          for (const [path, file] of Object.entries(result.files || {})) {
            const fileName = path.split('/').pop();
            const newFileName = `${treeId}_${fileName}`;
            zipFolder.file(newFileName, file);
          }
        }
        resolve();
      },
